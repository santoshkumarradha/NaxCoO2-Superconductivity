...
parser = argparse.ArgumentParser(
    description='Data file in xsf format to CUBE converter (the case of CRYSTAL)'
)
...
parser.add_argument("-t",
                    ...
                    dest="comment",
                    default="",
                    help="Comment to be added into output file")
parser.add_argument(
    "-s",
    "--supercell",
    action="store",
    ...
    dest="scell",
    default="444 555 1",
    help="Supercell size for automatic supercell generation in Jmol")
parser.add_argument(
    "-z",
    "--gzip",
    action="store_false",
    dest="tozip",
    help="Compress output. Warning unitcell data  won't be loaded in Jmol")
...
if (args.cube_fn == None):
    args.cube_fn = os.path.join(os.path.dirname(args.xsf_fn),
                                '%s.cube' % os.path.splitext(args.xsf_fn)[0])
    print(
        'Warning. No output filename was given. Will write to new one with .cube extension'
    )

if (os.path.isfile(args.cube_fn)):
    print(
        'Filename %s already exist. Remove old version and restart the program.'
        % args.cube_fn)
    sys.exit(1)
...
if (args.tozip):
    ...
    jmols = ''.join('%s%s%s%s%s%f%s' %
                    ('load "" {', args.scell, '} UNITCELL [', ','.join(
                        '%8.7f' % b for b in basis.flatten()),
                     ']; isosurface s1 sign cyan yellow cutoff ', args.cutoff,
                     ' "" color translucent 0.45'))
    ...
    cube_fh.write('%d %s\n' % (len(primcoord), ''.join(
        ' % 10.6f' % (o * Angst2Bohr) for o in origin)))
    # Write number of voxels and axis vector
    cube_fh.write('%5d %s\n' % (npntx, ''.join(
        ' % 10.6f' % (s / npntx * Angst2Bohr) for s in svec[0])))
    cube_fh.write('%5d %s\n' % (npnty, ''.join(
        ' % 10.6f' % (s / npnty * Angst2Bohr) for s in svec[1])))
    cube_fh.write('%5d %s\n' % (npntz, ''.join(
        ' % 10.6f' % (s / npntz * Angst2Bohr) for s in svec[2])))
    # Write atoms

    for i in range(len(primcoord)):
        cube_fh.write(
            '%3d %8.4f %s\n' %
            (primcoord[i][0], atom_data[primcoord[i][0]][3], ''.join(
                '% 10.6f' % (v * Angst2Bohr) for v in primcoord[i][1])))
