...
background (lithium cell) fills the same interlayer band and softens the well at fixed lattice
...
nor any verdict in Table~I, and the peak $T_c$ of 14~K (range 11--23~K, against the observed
4.5~K) fixes an order of magnitude rather than a precise value. Photoemission on the parent finds
bands narrower than ours by roughly a factor of two, raising $N(0)$ and lowering the coupling
estimate in proportion, so the dome is, if anything, conservative.
...
differ, so we compare topology and size rather than band identity). A further comparison is
suggestive: the parent shows a low-energy kink onsetting near 26~meV~\cite{Geck2007}, close to
the $\hbar\omega_{\rm eff}\approx23$~meV confined sodium mode predicted for the open gallery. The
parent's gallery is closed, so we do not identify the two; the coincidence instead motivates
...
to a fraction of a meV in the double well (SM~S6), and the Allen--Dynes dome of Fig.~2(c)
...
against the same cell with the water removed (construction, SM~S7). Figure~4(a) shows the
...
above (SM~S12). Na$_2$CoSe$_2$O~\cite{Cheng2024}, which superconducts at 6.3~K through its own
Co--Se network with no occupied gallery band, provides the negative control the picture requires.
...
\begin{table}[t]
  ...
  effect of Fig.~4 (SM~S7). Line-by-line derivations and the comparison with competing proposals
  are given in SM~S9.}
  \label{tab:score}
  ...
  \begin{tabular}{@{}p{6.0cm}@{}l@{}}
    ...
    Single $\Gamma$ barrel, no $e_g'$ pockets~\cite{Geck2007} & matched\\
    \bottomrule
  \end{tabular}
\end{table}
