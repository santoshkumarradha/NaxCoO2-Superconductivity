...
\begin{tikzpicture}[
  ...
  \foreach \g/\disp/\ne/\wat/\ox/\stg in {
    ...
    \begin{scope}[shift={(\ox,0)}]
      ...
      \ifnum\wat>0
        ...
        \node[water,font=\tiny,anchor=east] at (\W-0.05,\hg-0.20){H$_2$O};
      \fi
  ...
  \def\wy{-2.55}       % well-plot baseline (top of frame)
    ...
    \begin{scope}[shift={(\ox,\wy)}]
      ...
      \ifnum\lvl=2  % ---- soft double well ----
        \draw[ink,line width=1.1pt,smooth,domain=-1.28:1.28,samples=80]
          plot (\x,{-\wh + 0.35 + (-1.75*\x*\x + 1.15*\x*\x*\x*\x)});
        \pgfmathsetmacro\eb{-\wh+0.30}
        \draw[egas,line width=1.0pt] (-0.95,\eb)--(0.95,\eb);
      ...
      \ifnum\lvl=3  % ---- deep double well ----
        \draw[ink,line width=1.1pt,smooth,domain=-1.30:1.30,samples=90]
          plot (\x,{-\wh + 1.02 + (-4.6*\x*\x + 3.0*\x*\x*\x*\x)});
        \pgfmathsetmacro\ec{-\wh+0.28}
        \draw[egas,line width=1.0pt] (-1.02,\ec)--(1.02,\ec);
