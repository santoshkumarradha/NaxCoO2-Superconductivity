...
the warming branch of a thermal loop, rising near 146~K, collapsing near 240~K, and roughly
...
$0.39\pm0.05$~eV, which is the divacancy value in this material, and requires the onset to move
by $11\pm1$~K per decade of warming rate at fixed cooling protocol while the collapse stays
...
consistent with zero at one standard deviation, so that chain and the surface electron gas it
...
146~K, reaches a maximum comparable in size to the cooling baseline itself, and disappears
again near 240~K~\cite{Crowley2020,RadhaPerspective2021}. The loop repeats over thermal cycles
...
described with Fig.~\ref{fig:kinetics}; 146~K is the midpoint of the rise rather than its foot,
a distinction worth about 0.02~eV in the barrier extracted below, and the warming rate was not
...
nearer 185 and 200~K than the 146 and 240~K quoted here, and a larger anomaly obtained with a
...
and at the sensitivity $d\Ton/d\Ea=393$~K/eV derived in Sec.~\ref{sec:kinetics} the $\pm0.04$~eV
...
\begin{equation}
  ...
  \label{eq:Ea}
\end{equation}
an implicit relation solved by iteration in two or three steps. With $\Ton=146$~K, an attempt
...
Eq.~\eqref{eq:Ea} returns 0.450~eV if the growth velocity is taken to have saturated at the
onset. It has not saturated there. At 146~K the thermodynamic factor $1-e^{\Delta f/\kB T}$ of
Eq.~\eqref{eq:kjmarates} still stands at 0.14, and the transformation on the warming branch is
...
ramp. Retaining both corrections lowers the barrier to 0.387~eV at $n_{0}=10^{-2}$~nm$^{-2}$,
the two quantities entering together as
$\Ea=0.387+0.014\log_{10}(n_{0}/10^{-2}\,\mathrm{nm}^{-2})$~eV, so the barrier has to be quoted
...
decade of seed density, which combine in quadrature to $\Ea=0.39\pm0.05$~eV with a
corner-to-corner range of 0.27 to 0.46~eV; the accompanying $n_{0}$ runs from $10^{-4}$ to
...
environment-dependent scatter of $\pm0.04$~eV, and averages 0.36 to 0.39~eV over the composition
window $x=0.37$ to 0.50 that these flakes occupy, against 0.83~eV for the isolated-vacancy path
that operates only near full lithiation~\cite{VanderVen2001b}, so the extracted number
identifies the mechanism and not only the species. The identification should not be pressed
...
$\Ton=146$~K and $\Ea=0.39$~eV this is 4.4~K per $e$-fold. The thermodynamic factor of
Eq.~\eqref{eq:kjmarates} carries its own temperature dependence at the onset,
$d\ln f/dT=0.016$~K$^{-1}$ there, which steepens the slope by 7.7~percent, slightly larger than
the $2\kB T/\Ea$ correction the Laplace evaluation already retains and of the opposite sign; keeping
both gives 11.1~K per decade of warming rate, and across the barrier range admitted above the
prediction is $11\pm1$~K per decade.
...
16.5~K per decade, so a series run without that precaution measures a number whose
...
percent range over 7.3~K. The measured width, $19^{+6}_{-9}$~K, sits well above that, and a
spread of barriers of about $\pm0.045$~eV closes the gap at the sensitivity
$d\Ton/d\Ea=393$~K/eV, which compositional inhomogeneity of the kind seen across
...
only 1.7 to 20~nm to reach half conversion at 146~K, so a nucleus born at the nose would
...
base temperature must have been born below $\Ton-9$~K, which is below about 137~K.

Classical nucleation cannot put them there. Forcing the maximum of $I_{\mathrm{n}}$ below 137~K
demands $W^{*}/\kB T\simeq100$ at that temperature, which suppresses the rate by $e^{-100}$, and
...
with no mobility factor to pay. At 137~K that radius is 0.7 to 1.3~nm, twenty to eighty cobalt
sites, and the assumption carries an explicit condition that should be stated with it: the
embryo distribution must be cut off above about 1.3~nm, since one 2~nm embryo anywhere in the
...
$n_{0}\propto1/|r_{\mathrm{cool}}|$ and would raise the onset by 5.5~K per decade of cooling
...
branch needs divided by the growth velocity, so a hold at 135 to 140~K, six to eleven kelvin
below the onset, should complete in between three minutes and two hours, centrally between
fifteen and fifty minutes, and a hold that produces nothing there is genuinely falsifying. A
...
The collapse places $\Tcol(0)=240\pm10$~K for this flake, and the bulk comparison has to be made
against the right scale, because bulk Li$_x$CoO$_2$ carries two. The lower one, near 130~K, is
...
Mapped onto that pair, the flake's kinetic edge at 146~K and the bulk kinetic scale agree with
...
\begin{figure}[!tbp]
  ...
  cooling and a warming sweep with $\Ea=0.39$~eV, $\nu=10^{13}$~s$^{-1}$ and $\Tcol=240$~K, and
  ...
  parameters listed above. The 5~T trace of the same figure is drawn for reference and is not
  ...
  percent rise width of the 1~K/min warming curve. The model peak sits somewhat above the
  ...
  the thermodynamic factor retained, giving 11~K per decade at $\Ea=0.39$~eV and containing no
...
established. The consequences drawn below therefore follow from a shift that a field series has
still to confirm. What the present data do establish robustly is
...
anisotropy rather than by their size. A field that enhances the order parameter or aligns
...
where it is while thermally born ones raise it by 5.5~K per decade; a field series at 1, 2, 3,
...
condition fixes an effective barrier of $0.39\pm0.05$~eV, together with a seed density between
$10^{-4}$ and $10^{-1}$~nm$^{-2}$, which identifies the divacancy hop as the mechanism and
predicts an onset shift of $11\pm1$~K per decade of warming rate at fixed cooling protocol,
...
correlated moments; the measured shift is consistent with zero at one standard deviation, so
that whole chain waits on a field series.
