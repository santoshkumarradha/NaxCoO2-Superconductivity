...
nc = 10
nd = 10
c_axis = np.linspace(1.2, 1.2 + .7, nc)
d_move = np.linspace(0, 1.2, nd)
...
#-----------load Main structure---------------------------------------------------
parser = CifParser("structures_files/nacoo2.cif")
...
for c in c_axis:
    for d in d_move:
        ...
        if struc not in done_structures:
            ...
            fname = "./" + str(np.round(c, 4)) + "_caxis/" + str(np.round(d, 4))
            Path(fname).mkdir(parents=True, exist_ok=True)
            print("running c={} and d= {}".format(struc.lattice.c, d))
            with cd(fname):
                CifWriter(struc).write_file('licoo2_structure.cif')
                calculator = lmf(nkabc=[4, 4, 4],
                                 ctrl="temp",
                                 p=num_processors,
                                 silent=0)
