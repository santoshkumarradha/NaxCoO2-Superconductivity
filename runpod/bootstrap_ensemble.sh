#!/bin/bash
# Pod bootstrap for the "cage-ensemble" set L/M campaign.
# Image: nvcr.io/hpc/quantum_espresso:qe-7.3.1 -- bare Ubuntu with QE
# binaries preinstalled at /usr/local/qe/bin (PATH/LD_LIBRARY_PATH exported
# in run_ensemble.sh), but no ssh/git: install those first, unlock root
# (image ships root locked), inject the runpod-generated $PUBLIC_KEY, clone
# the repo, and launch the pipeline.
apt-get update -qq && apt-get install -y -qq curl openssh-server git rsync python3-numpy >/dev/null 2>&1
mkdir -p /run/sshd /root/.ssh
echo 'ssh-rsa  runpodctl-ssh-key' >> /root/.ssh/authorized_keys
[ -n "$PUBLIC_KEY" ] && echo "$PUBLIC_KEY" >> /root/.ssh/authorized_keys
usermod -p '*' root
echo 'ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIIWsQNj8wVSSCkStELR4hMyiP8xm49qjTopL+IdmG9Yw runpod-access' >> /root/.ssh/authorized_keys
echo 'ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAICARC8tdRQcP2eIR0zcjnyMEmLO4PxMQF8k+mE0f0RY7 santosh@agnostiq.ai' >> /root/.ssh/authorized_keys
chmod 700 /root/.ssh && chmod 600 /root/.ssh/authorized_keys
/usr/sbin/sshd
git clone https://github.com/santoshkumarradha/NaxCoO2-Superconductivity /workspace/repo
cd /workspace/repo/runpod && nohup bash run_ensemble.sh > run_ensemble.log 2>&1 &
sleep infinity
