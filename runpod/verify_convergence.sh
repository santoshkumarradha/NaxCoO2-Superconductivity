...
for z in 0.90 1.02; do
  zo=$(python3 -c "print(f'{$z/6.9:.6f}')"); zt=$(python3 -c "print(f'{1-$z/6.9:.6f}')")
  run zO${z}_c6.9 Na_1x1_c6.9_d0.50 "s/^O .* 0\.13913.*/O 0.333333 0.666667 $zo/; s/^O .* 0\.86086.*/O 0.666667 0.333333 $zt/"
done
echo "SET-D COMPLETE"
