#!/usr/bin/env bash
# GPU-parallel launcher for the water-landscape job set W in jobs_wscan/.
# 16 relax jobs: sqrt3 x sqrt3 Na_1/3CoO2 + 4 H2O at c = 9.9 A, Na at
# delta = 0.30 A (12 configs) and 0.00 A (4 controls), each with a
# different seeded-random H2O orientation start, relaxed to full BFGS
# convergence (nstep = 250).
...
#
# There is NO PP-tools bootstrap here -- set W is pw.x-only (one relax per job).
set -uo pipefail
...
[ -f manifest_wscan.json ] || python3 generate_inputs.py --stage wscan
...
run_job() {  # $1 = gpu id, $2 = job name
  local gpu="$1" name="$2" dir="jobs_wscan/$2"
...
list_jobs() {  # all set-K job names, manifest order
  python3 - <<'EOF'
import json
for j in json.load(open("manifest_wscan.json"))["jobs"]:
    print(j["name"])
EOF
}

echo "== set W: water-landscape relax jobs =="
list_jobs > .queue_wscan
run_pool .queue_wscan
[ -f STOP ] && { echo "stopped by STOP file"; exit 1; }

echo "== all wscan-set jobs finished =="
echo "Summary:"
n_ok=0; n_bad=0
for d in jobs_wscan/*/; do
  if job_done "$d"; then n_ok=$((n_ok+1)); else n_bad=$((n_bad+1)); echo "  incomplete: $d"; fi
done
echo "  $n_ok done, $n_bad incomplete"
echo "  analyze: collect final converged total energies and Na z per config;"
echo "  the spread/multiplicity across configs at fixed delta quantifies the"
echo "  water hydrogen-bond landscape roughness; see manifest_wscan.json."
echo "WSCAN COMPLETE"
